...
<div class="w-full max-w-6xl mx-auto px-6 sm:px-8 lg:px-12 py-20">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-12">
        <div class="flex flex-wrap items-center gap-3">
            <button
                type="button"
                class="px-6 py-3 rounded-full text-base md:text-lg font-bold transition-all duration-300 shadow-sm {activeBoard === null ? 'bg-primary-900 text-white shadow-lg scale-105' : 'bg-white text-gray-600 border-2 border-gray-100 hover:border-primary-300 hover:text-primary-900'}"
                onclick={() => selectCategory(null)}
            >전체</button>
            {#each data.categories as category}
                <button
                    type="button"
                    class="px-6 py-3 rounded-full text-base md:text-lg font-bold transition-all duration-300 shadow-sm {activeBoard === category.id ? 'bg-primary-900 text-white shadow-lg scale-105' : 'bg-white text-gray-600 border-2 border-gray-100 hover:border-primary-300 hover:text-primary-900'}"
                    ...
                    >
                        <div class="flex flex-col md:flex-row gap-4 md:items-center justify-between">
                            <div class="flex items-center gap-6">
                                <span class="shrink-0 px-4 py-2 bg-primary-50 text-primary-700 text-sm md:text-base font-bold rounded-lg border border-primary-100 w-24 text-center">
                                    {item.category}
                                </span>
                                <h3 class="text-xl md:text-2xl font-semibold text-gray-900 transition-colors line-clamp-1">
                                    {item.title}
                                </h3>
                                {#if item.isNew}
                            ...
                            <div class="flex items-center gap-2 text-base text-gray-500 font-medium md:ml-auto pl-28 md:pl-0">
