<script lang="ts">
    ...
    let categoryId = $state<number>(data.categories[0]?.id ?? 7);
    ...
    async function handleSubmit(e: Event) {
        ...
        if (MANAGER_ONLY_CATEGORY_IDS.includes(categoryId) && userLevel < MANAGER_LEVEL) {
            error = '해당 분류는 매니저 이상만 작성할 수 있습니다.';
            return;
        }
        ...
        if (mediaUrls.length > 0) {
            await supabaseBrowser.from('posts').update({ media_urls: mediaUrls }).eq('id', post.id);
        }
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
                        {#each images as img, i}
                            <div class="relative group aspect-square rounded-xl overflow-hidden border-2 border-gray-100 bg-gray-50">
                                ...
                                <button type="button" onclick={() => removeImage(i)} aria-label="이미지 삭제"
                ...
                <div class="space-y-2">
                    {#each urlItems as item, i}
                        <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 group">
                            ...
                            <button type="button" onclick={() => removeUrl(i)} aria-label="URL 삭제"
