...
<div class="bg-gradient-to-b from-primary-50 to-white py-10 md:py-20 border-b border-gray-100">
    <div class="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 text-center animate-fade-in-up">
        <h1 class="text-3xl md:text-6xl font-black text-gray-900 mb-4 md:mb-8 tracking-tight font-serif drop-shadow-sm">게시판</h1>
...
<div class="w-full max-w-6xl mx-auto px-6 sm:px-8 lg:px-12 py-12 md:py-20">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 md:mb-12">
        ...
        >
            <option value="" selected={data.categoryId === null}>전체</option>
        ...
        <div class="hidden md:flex items-center gap-3 py-1 px-0.5 shrink-0">
            <button
                type="button"
                class="whitespace-nowrap px-6 py-3 rounded-full text-lg font-bold transition-all duration-300 shadow-sm {data.categoryId === null ? 'bg-primary-900 text-white shadow-lg scale-105' : 'bg-white text-gray-600 border-2 border-gray-100 hover:border-primary-300 hover:text-primary-900'}"
                onclick={() => selectCategory(null)}
            >전체</button>
            {#each data.categories as cat}
                <button
                    type="button"
                    class="whitespace-nowrap px-6 py-3 rounded-full text-lg font-bold transition-all duration-300 shadow-sm {data.categoryId === cat.id ? 'bg-primary-900 text-white shadow-lg scale-105' : 'bg-white text-gray-600 border-2 border-gray-100 hover:border-primary-300 hover:text-primary-900'}"
